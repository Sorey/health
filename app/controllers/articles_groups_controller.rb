class ArticlesGroupsController < ApplicationController
  ...
  def index
    @articles_groups = ArticlesGroup.all
  end
  ...
  def new
    @articles_group = ArticlesGroup.new
  end
  ...
  def create
    @articles_group = ArticlesGroup.new(articles_group_params)
  ...
  private
    ...
    def set_articles_group
      @articles_group = ArticlesGroup.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def articles_group_params
      params.require(:articles_group).permit(:title, :description, :publish_on)
    end
end
